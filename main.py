...
class OrcaClient():
    ...
    def __init__(self, pool="devUSDC/devUSDT", params={}):
        self.pools = json.load(open("data/pools.json", "r"))
        self.pool = self.pools[pool]
        default_params = {"rpc": "https://api.devnet.solana.com", "config": "FcrweFY1G9HJAHG5inkGB6pKg1HZ6x9UC2WioAfWrGkR"}
        self.params = params
        for key in default_params:
            if (key not in self.params):
                self.params[key] = default_params[key]
        self.keypair = Keypair.from_secret_key(bytes(json.load(open("data/wallet.json"))))
        
        connection = AsyncClient(self.params["rpc"])
        self.ctx = WhirlpoolContext(self.params["config"], connection, self.keypair)

    async def get_whirlpool(self):

        whirlpool_pubkey = PDAUtil().get_whirlpool(
            ORCA_WHIRLPOOL_PROGRAM_ID, # program id
            PublicKey(self.params["config"]), # whirlpools config
            PublicKey(self.pool["a_mint"]),
            PublicKey(self.pool["b_mint"]),
            self.pool["ticks"]
        ).pubkey
        whirlpool = await self.ctx.fetcher.get_whirlpool(whirlpool_pubkey)

        print("kepair:", self.keypair.public_key)
        print("whirlpool_pubkey:", whirlpool_pubkey)
        print("whirlpool:", whirlpool)

        token_a_decimal = (await self.ctx.fetcher.get_token_mint(whirlpool.token_mint_a)).decimals  # SOL_DECIMAL
        token_b_decimal = (await self.ctx.fetcher.get_token_mint(whirlpool.token_mint_b)).decimals  # USDC_DECIMAL
    ...
    def test(self):
        ...
        token_account_a = TokenUtil.derive_ata(self.keypair.public_key, whirlpool.token_mint_a)
        token_account_b = TokenUtil.derive_ata(self.keypair.public_key, whirlpool.token_mint_b)
        ...
        # get Oracle
        oracle = PDAUtil.get_oracle(self.ctx.program_id, whirlpool_pubkey).pubkey
        print("oracle", oracle)
        ...
        ix = WhirlpoolIx.swap(
            ...
            SwapParams(
                ...
                a_to_b=a_to_b,
                token_authority=self.keypair.public_key,
        ...
        tx = TransactionBuilder(self.ctx.connection, self.keypair).add_instruction(ix)
...
if __name__ == "__main__":
    ...
    client = OrcaClient()
    asyncio.run(client.get_whirlpool())
